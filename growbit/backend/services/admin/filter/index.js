...
const adminGetFilterListSocket = async (io, socket, user, data, callback) => {
  try {
    ...
    const dataDatabase = await Promise.all([
      FilterPhrase.countDocuments({
        phrase: { $regex: data.search, $options: "i" },
      }),
      FilterPhrase.find({ phrase: { $regex: data.search, $options: "i" } })
        .limit(12)
        .skip(offset)
        .select("phrase createdAt")
        .lean(),
    ]);
...
const adminSendFilterCreateSocket = async (
  io,
  socket,
  user,
  data,
  ...
  try {
    ...
    let filterDatabase = await FilterPhrase.findOne({ phrase: phrase })
      .select("phrase")
      .lean();
...
const adminSendFilterRemoveSocket = async (
  io,
  socket,
  user,
  data,
  ...
  try {
    ...
    let filterDatabase = await FilterPhrase.findById(data.filterId)
      .select("phrase")
      .lean();
