...
const socketCheckUserData = (user, checkAuth) => {
  if (checkAuth === true && user === null) {
    ...
    checkAuth === true &&
    user.ban !== undefined &&
    new Date(user.ban.expire).getTime() > new Date().getTime()
  ) {
    throw new Error(
      `You got banned for ${user.ban.type} because of the reason: ${user.ban.reason}.`,
    );
  }
};
...
const socketCheckConnectionLimit = (room, identifier) => {
  return new Promise(async (resolve, reject) => {
    try {
      const count = socketActiveConnections[room].filter(
        (element) => element === identifier.toString(),
      ).length;

      if (count >= 5) {
        throw new Error("Too many active socket connections.");
      } else {
        resolve();
      }
    } catch (err) {
...
const socketRemoveConnectionLimit = (room, identifier) => {
  const index = socketActiveConnections[room].findIndex(
    (element) => element === identifier.toString(),
  );
  if (index !== -1) {
    socketActiveConnections[room].splice(index, 1);
  }
};
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (socketActiveRequests.includes(identifier.toString()) === true) {
        throw new Error(
          "You need to slow down, we still processing your last request.",
        );
      } else {
