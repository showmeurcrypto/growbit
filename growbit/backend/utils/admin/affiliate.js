...
const adminChecksetAffiliateCodeData = (data) => {
  if (data === undefined || data === null) {
    throw new Error("Something went wrong. Please try again in a few seconds.");
  } else if (
    data.userId === undefined ||
    ...
    (data.code !== null &&
      (typeof data.code !== "string" ||
        ...
        data.code.length <= 3 ||
        data.code.length > 20 ||
        validator.isAlphanumeric(data.code, "en-US", { ignore: "-_" }) !==
          true))
  ) {
...
const adminChecksetAffiliateCodeCode = (codeDatabase) => {
  if (codeDatabase !== null) {
    throw new Error(
      "Your entered affiliate code is already used by a other user.",
    );
  }
};
