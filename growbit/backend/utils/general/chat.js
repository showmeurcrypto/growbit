...
const generalCheckGetChatMessagesData = (data) => {
  if (data === undefined || data === null) {
    ...
    ["en", "tr", "de", "es", "beg", "whale"].includes(data.room) === false
  ) {
...
const generalCheckSendChatMessageRoom = (user, settings, chatRoom) => {
  if (chatRoom === undefined || chatRoom === null) {
    ...
    settings.chat.rooms[chatRoom].enabled !== true
  ) {
    throw new Error("Your entered chat room is currently locked.");
  } else if (chatRoom === "whale" && user.stats.bet < 1000000 * 1000) {
    throw new Error(
      "Your need to have a minimum wager of 1,000,000 to use the whale chat.",
    );
  }
};
...
const generalCheckSendChatMessageUser = (user, settings, chatUserCooldowns) => {
  if (
    user === undefined ||
    settings === undefined ||
    chatUserCooldowns === undefined
  ) {
    throw new Error("Something went wrong. Please try again in a few seconds.");
  } else if (
    user.mute !== undefined &&
    user.mute.expire !== undefined &&
    new Date(user.mute.expire).getTime() > Date.now()
  ) {
    throw new Error(
      `You are still muted for another ${generalChatGetMuteTime(user.mute.expire)}.`,
    );
  } else if (user.rank === "user" && user.stats.deposit < 5) {
    throw new Error(`You need to deposit at least 5DL to chat.`);
  } else if (
    settings.chat.mode === "normal" &&
    user.rank === "user" &&
    chatUserCooldowns[user._id.toString()] !== undefined &&
    chatUserCooldowns[user._id.toString()] >
      Math.floor(new Date().getTime() - 3000)
  ) {
    throw new Error(`You can only send 1 message every 3 seconds.`);
  } else if (
    settings.chat.mode === "slow" &&
    user.rank === "user" &&
    chatUserCooldowns[user._id.toString()] !== undefined &&
    chatUserCooldowns[user._id.toString()] >
...
const generalCheckSendChatRemoveData = (data) => {
  if (data === undefined || data === null) {
    ...
    validator.isMongoId(data.messageId) !== true
  ) {
    throw new Error("Your entered chat message is invalid.");
  }
};
...
const generalCheckSendChatLockRoom = (data, settings, chatRoom) => {
  if (chatRoom === undefined || chatRoom === null) {
    ...
    throw new Error(
      `Your entered chat room is already ${data.value === true ? "unlocked" : "locked"}.`,
    );
  }
};
...
const generalGetChatOnlineCount = (io) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        en: Math.floor(onlineData[0].size),
...
const generalChatGetMuteTime = (muteExpire) => {
  const timeLeft = Math.floor(
    (new Date(muteExpire).getTime() - new Date().getTime()) / 1000,
  );
...
const generalFilterMessage = (message, generalChatFilter) => {
  ...
  for (let phrase of generalChatFilter) {
    ...
    while (lower.includes(phrase) === true) {
      ...
      lower =
        lower.substring(0, pos) +
        "*".repeat(phrase.length) +
        lower.substring(pos + phrase.length);
      escaped =
        escaped.substring(0, pos) +
        "*".repeat(phrase.length) +
        escaped.substring(pos + phrase.length);
    }
  }
