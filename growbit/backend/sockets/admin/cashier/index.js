...
} = require("../../../services/admin/cashier");
...
module.exports = (io, socket) => {
    ...
    if (socket.decoded !== undefined && socket.decoded !== null) {
      try {
        ...
        try {
          ...
          socketCheckUserData(user, true);
          socketCheckUserRank(user, ["admin"]);
          await adminAproveWithdrawCryptoByOxapay(
            io,
            socket,
            user,
            data,
  ...
  socket.on("sendCashierMmoAction", async (data, callback) => {
    if (callback === undefined || typeof callback !== "function") {
      return;
    }
    if (socket.decoded !== undefined && socket.decoded !== null) {
      try {
        ...
        try {
          ...
          socketCheckUserData(user, true);
          socketCheckUserRank(user, ["admin"]);
          adminSendCashierMmoActionSocket(io, socket, user, data, callback);
        } catch (err) {
          socketRemoveAntiSpam(socket.decoded._id);
          callback({
            success: false,
