<template>
  <div class="admin-users">
    <div class="users-list">
      ...
      <div class="list-content">
        <transition name="fade" mode="out-in">
          <div
            v-if="adminUserList.data === null || adminUserList.loading === true"
          ...
          <div
            v-else-if="adminUserList.data.length > 0"
            class="content-list"
            key="data"
          >
            <AdminUsersElement
              v-for="user in adminUserList.data"
              v-bind:key="user._id"
              v-bind:user="user"
            />
          </div>
      ...
      <div class="list-pagination">
        <button
          v-on:click="adminSetPage(adminUserList.page - 1)"
          class="button-prev"
          v-bind:disabled="adminUserList.page <= 1"
        >
        ...
        <div class="pagination-info">
          PAGE
          <span class="text-green-gradient">{{ adminUserList.page }}</span> /
          {{
            Math.ceil(adminUserList.count / 12) <= 0
              ? "1"
              : Math.ceil(adminUserList.count / 12)
          }}
        </div>
        <button
          v-on:click="adminSetPage(adminUserList.page + 1)"
          class="button-next"
          v-bind:disabled="
            adminUserList.page >= Math.ceil(adminUserList.count / 12)
          "
        >
          <div class="button-inner">
            <svg
    ...
    <div class="users-filters">
      ...
      <div class="ban">
        <input v-model="ip" />
        <button @click="banIp()">Ban IP</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    adminSetPage(page) {
      if (this.adminUserList.page === page) {
        return;
      }
      if (page < 1 || page > Math.ceil(this.adminUserList.count / 12)) {
        return;
      }
      ...
      const data = {
        page: this.adminUserList.page,
        search: this.adminFilterSearch,
        sort: this.adminFilterSort.toLowerCase(),
      };
      this.adminGetUserListSocket(data);
    },
    detectMultis() {
      this.socketAdmin.emit("detectMultis", {}, (res) => {
        if (res.success === true) {
    ...
    banIp() {
      this.socketAdmin.emit("banIp", { ip: this.ip }, (res) => {
        if (res.success === true) {
          this.notificationShow({
            type: "success",
            message: `${res.banned} users have been banned!`,
          });
        } else {
          this.notificationShow(res.error);
        }
