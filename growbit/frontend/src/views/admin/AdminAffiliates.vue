    ...
    <div class="affiliates-list">
      ...
      <div class="list-content">
        <transition name="fade" mode="out-in">
          <div
            v-if="
              adminAffiliateList.data === null ||
              adminAffiliateList.loading === true
            "
          ...
          <div
            v-else-if="adminAffiliateList.data.length > 0"
            class="content-list"
            key="data"
          >
            <AdminAffiliatesElement
              v-for="affiliate in adminAffiliateList.data"
              v-bind:key="affiliate._id"
              v-bind:affiliate="affiliate"
            />
          </div>
      ...
      <div class="list-pagination">
        <button
          v-on:click="adminSetPage(adminAffiliateList.page - 1)"
          class="button-prev"
          v-bind:disabled="adminAffiliateList.page <= 1"
        >
        ...
        <div class="pagination-info">
          PAGE
          <span class="text-green-gradient">{{ adminAffiliateList.page }}</span>
          /
          {{
            Math.ceil(adminAffiliateList.count / 12) <= 0
              ? "1"
              : Math.ceil(adminAffiliateList.count / 12)
          }}
        </div>
        <button
          v-on:click="adminSetPage(adminAffiliateList.page + 1)"
          class="button-next"
          v-bind:disabled="
            adminAffiliateList.page >= Math.ceil(adminAffiliateList.count / 12)
          "
        >
          <div class="button-inner">
            <svg
...
export default {
  ...
  methods: {
    ...
    adminSetPage(page) {
      if (this.adminAffiliateList.page === page) {
        return;
      }
      if (page < 1 || page > Math.ceil(this.adminAffiliateList.count / 12)) {
        return;
      }
      ...
      const data = {
        page: this.adminAffiliateList.page,
