  ...
  <div class="admin-leaderboards">
    <div class="leaderboards-list">
      ...
      <div class="list-content">
        <transition name="fade" mode="out-in">
          <div
            v-if="
              adminLeaderboardList.data === null ||
              adminLeaderboardList.loading === true
            "
          ...
          <div
            v-else-if="adminLeaderboardList.data.length > 0"
            class="content-list"
            key="data"
          >
            <AdminLeaderboardElement
              v-for="leaderboard in adminLeaderboardList.data"
              v-bind:key="leaderboard._id"
      ...
      <div class="list-pagination">
        <button
          v-on:click="adminSetPage(adminLeaderboardList.page - 1)"
          class="button-prev"
          v-bind:disabled="adminLeaderboardList.page <= 1"
        >
        ...
        <div class="pagination-info">
          ...
          <span class="text-green-gradient">{{
            adminLeaderboardList.page
          }}</span>
          /
          {{
            Math.ceil(adminLeaderboardList.count / 12) <= 0
              ? "1"
              : Math.ceil(adminLeaderboardList.count / 12)
          }}
        </div>
        <button
          v-on:click="adminSetPage(adminLeaderboardList.page + 1)"
    ...
    <div class="leaderboards-filters">
      <div class="filters-create">
        ...
        <button @click="adminCreateButton()" class="button-create">
          Create Race
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    adminSetPage(page) {
      if (this.adminLeaderboardList.page === page) {
        return;
      }
      if (page < 1 || page > Math.ceil(this.adminLeaderboardList.count / 12)) {
        return;
      }
    ...
    adminCreateButton() {
      const data = {
        type: this.adminFilterType.toLowerCase(),
  ...
  created() {
    if (this.adminLeaderboardList.loading === false) {
      const data = {
        page: this.adminLeaderboardList.page,
        search: this.adminFilterSearch,
      };
      this.adminGetLeaderboardListSocket(data);
    }
...
.admin-leaderboards .filters-create button.button-create {
  ...
  box-shadow: 0 2px 0 0 rgba(91, 70, 188, 0.6);
}
