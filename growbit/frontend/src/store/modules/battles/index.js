...
const mutations = {
  ...
  battles_update_games(state, game) {
    state.battlesGames.splice(
      state.battlesGames.findIndex((element) => element._id === game._id),
      1,
      game
    );
  },
...
const actions = {
  ...
  battlesRemoveSelected({ getters, commit }, box) {
    const index = getters.battlesSelected.findIndex(
      (element) => element._id === box._id
    );
    if (index !== -1) {
      commit("battles_remove_selected", index);
    }
  },
  ...
  battlesSocketGame({ getters, commit }, data) {
    if (data.game.options.private === false) {
      if (data.game.state !== "completed") {
        if (
          getters.battlesGames.some(
            (element) => element._id === data.game._id
          ) === true
        ) {
          commit("battles_update_games", data.game);
        } else {
          commit("battles_add_games", data.game);
        }
      } else {
        commit("battles_remove_games", data.game);
        commit("battles_add_history", data.game);
        if (getters.battlesHistory.length > 4) {
          commit("battles_remove_last_history");
        }
      }
    }

    if (
      getters.battlesGameData.game !== null &&
      getters.battlesGameData.game._id === data.game._id
    ) {
      commit("battles_set_game_data_game", {
        ...data.game,
        boxes: getters.battlesGameData.game.boxes,
      });
    }
  },
  battlesGetGameDataSocket({ getters, commit, dispatch }, data) {
    if (
      getters.socketBattles === null ||
      getters.battlesGameData.loading === true
    ) {
      return;
    }
  ...
  battlesSendCreateSocket({ getters, commit, dispatch }, data) {
    if (getters.socketBattles === null || getters.socketSendLoading !== null) {
      return;
    }
    commit("socket_set_send_loading", "BattlesCreate");

    getters.socketBattles.emit("create", data, (res) => {
      if (res.success === true) {
  ...
  battlesSendBotSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketBattles.emit("callBot", data, (res) => {
      if (res.success === true) {
      } else {
        dispatch("notificationShow", res.error);
      }
  ...
  battlesSendJoinSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketBattles.emit("join", data, (res) => {
      if (res.success === true) {
