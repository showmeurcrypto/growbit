...
const getters = {
  ...
  supportMessages: (state) => state.supportChat,
};
...
const mutations = {
  ...
  support_chat_set_messages_data(state, data) {
    state.supportChat.data = data;
  },
  support_chat_add_messages_data(state, message) {
    state.supportChat.data.push(message);
  },
  chat_remove_messages_data(state, messageId) {
    state.chatMessages.data.splice(
      state.chatMessages.data.findIndex((element) => element._id === messageId),
      1
    );
  },
...
const actions = {
  ...
  chatSocketMessage({ getters, commit }, data) {
    ...
    if (getters.chatMessages.data.length > 50 && getters.chatScroll === true) {
      commit("chat_remove_messages_data_overdue");
    }
  },
  ...
  chatGetMessagesSocket({ getters, commit, dispatch }, data) {
    if (
      ...
      getters.chatMessages.loading !== false
    ) {
      return;
    }
  ...
  chatIgnoreUserSocket({ getters, commit, dispatch }, data) {
    if (getters.socketGeneral === null || getters.socketSendLoading !== null) {
      return;
    }
    commit("socket_set_send_loading", "ChatIgnore");

    getters.socketGeneral.emit("ignoreUser", data, (res) => {
      if (res.success === true) {
        dispatch("notificationShow", {
          type: "success",
          message: "Done!",
        });
        commit("update_ignore_list", res.user);
        dispatch("modalsSetShow", null);
      } else {
        dispatch("notificationShow", res.error);
      }
  ...
  supportChatSendMessage({ getters, commit, dispatch }, data) {
    if (getters.supportChatSocket === null) {
      return;
    }

    getters.supportChatSocket.emit(
      "send",
      {
      ...
      (res) => {
        if (res.success) {
          commit("support_chat_add_messages_data", {
            message: data.message,
            sender: "You",
          });
        } else {
