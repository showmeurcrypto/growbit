const state = {
  unboxFilterSearch: "",
  unboxFilterSort: "ascending",
  unboxFilterVolatility: "all",
...
const getters = {
  ...
  unboxFilterVolatility: (state) => state.unboxFilterVolatility,
  ...
  unboxQuick: (stata) => state.unboxQuick,
};
...
const mutations = {
  ...
  unbox_set_filter_volatility(state, value) {
    state.unboxFilterVolatility = value;
  },
  ...
  unbox_toggle_quick(state, status) {
    state.unboxQuick = status;
  },
};

const actions = {
  unboxToggleQuick({ commit, getters }) {
    commit("unbox_toggle_quick", !getters.unboxQuick);
  },
  ...
  unboxSetFilterVolatility({ commit }, value) {
    commit("unbox_set_filter_volatility", value);
  },
  ...
  unboxGetBoxDataSocket({ getters, commit, dispatch }, data) {
    if (getters.socketUnbox === null || getters.unboxBoxData.loading === true) {
      return;
    }
  ...
  unboxSendBetSocket({ getters, commit, dispatch }, data) {
    if (getters.socketUnbox === null || getters.socketSendLoading !== null) {
      return;
    }
    commit("socket_set_send_loading", "UnboxBet");

    getters.socketUnbox.emit("unbox", data, (res) => {
      if (res.success === true) {
  ...
  unlockWithTheKey({ getters, commit, dispatch }, data) {
    if (getters.socketUnbox === null || getters.socketSendLoading !== null) {
      return;
    }
    commit("socket_set_send_loading", "UnboxBet");

    getters.socketUnbox.emit("unlockWithTheKey", data, (res) => {
      if (res.success === true) {
        // commit("auth_update_user", res.user);
        commit("unbox_set_games", res.games);
        commit("unbox_set_running", true);
        commit("add_claimed_key", res.key);
      } else {
        dispatch("notificationShow", res.error);
      }
