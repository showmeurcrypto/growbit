...
const mutations = {
  ...
  crash_update_bets(state, bet) {
    state.crashBets.splice(
      state.crashBets.findIndex((element) => element._id === bet._id),
      1,
      bet
    );
  },
...
const actions = {
  ...
  crashSocketGame({ commit, dispatch }, data) {
    commit("crash_set_game", data.game);
    //console.log("crash socket");
  ...
  crashSocketBet({ getters, commit }, data) {
    // console.log("new  crash bet");
    // console.log(data.bet);
    if (
      getters.crashBets.some((element) => element._id === data.bet._id) === true
    ) {
