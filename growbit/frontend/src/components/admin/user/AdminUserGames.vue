  ...
  <div class="admin-user-games">
    ...
    <div class="games-content">
      <transition name="fade" mode="out-in">
        <div
          v-if="
            adminUserGameList.game === null ||
            adminUserGameList.loading === true
          "
        ...
        <div
          v-else-if="adminUserGameList.data.length > 0"
          class="content-list"
          key="data"
        >
          <AdminUserGamesElement
            v-for="game in adminUserGameList.data"
            v-bind:key="game._id"
            v-bind:game="game"
          />
        </div>
    ...
    <div class="games-pagination">
      <button
        v-on:click="adminSetPage(adminUserGameList.page - 1)"
        class="button-prev"
        v-bind:disabled="adminUserGameList.page <= 1"
      >
      ...
      <div class="pagination-info">
        PAGE
        <span class="text-green-gradient">{{ adminUserGameList.page }}</span> /
        {{
          Math.ceil(adminUserGameList.count / 14) <= 0
            ? "1"
            : Math.ceil(adminUserGameList.count / 14)
        }}
      </div>
      <button
        v-on:click="adminSetPage(adminUserGameList.page + 1)"
        class="button-next"
        v-bind:disabled="
          adminUserGameList.page >= Math.ceil(adminUserGameList.count / 14)
        "
      >
        <div class="button-inner">
          <svg
...
export default {
  ...
  methods: {
    ...mapActions(["adminSetUserGameListPage", "adminGetUserGameListSocket"]),
    adminSetPage(page) {
      if (this.adminUserGameList.page === page) {
        return;
      }
      if (page < 1 || page > Math.ceil(this.adminUserGameList.count / 14)) {
        return;
      }
      ...
      const data = {
        ...
        page: this.adminUserGameList.page,
      };
      this.adminGetUserGameListSocket(data);
    },
  },
  computed: {
    ...mapGetters(["modalsShow", "modalsData", "adminUserGameList"]),
  },
  created() {
