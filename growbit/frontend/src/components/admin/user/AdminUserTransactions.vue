  ...
  <div class="admin-user-transactions">
    ...
    <div class="transactions-content">
      <transition name="fade" mode="out-in">
        <div
          v-if="
            adminUserTransactionList.data === null ||
            adminUserTransactionList.loading === true
          "
        ...
        <div
          v-else-if="adminUserTransactionList.data.length > 0"
          class="content-list"
          key="data"
        >
          <AdminUserTransactionsElement
            v-for="transaction in adminUserTransactionList.data"
    ...
    <div class="transactions-pagination">
      <button
        v-on:click="adminSetPage(adminUserTransactionList.page - 1)"
        class="button-prev"
        v-bind:disabled="adminUserTransactionList.page <= 1"
      >
      ...
      <div class="pagination-info">
        ...
        <span class="text-green-gradient">{{
          adminUserTransactionList.page
        }}</span>
        /
        {{
          Math.ceil(adminUserTransactionList.count / 14) <= 0
            ? "1"
            : Math.ceil(adminUserTransactionList.count / 14)
        }}
      </div>
      <button
        v-on:click="adminSetPage(adminUserTransactionList.page + 1)"
...
export default {
  ...
  methods: {
    ...
    adminSetPage(page) {
      if (this.adminUserTransactionList.page === page) {
        return;
      }
      if (
        page < 1 ||
        page > Math.ceil(this.adminUserTransactionList.count / 14)
      ) {
        return;
      }
      ...
      const data = {
        ...
        page: this.adminUserTransactionList.page,
      };
      this.adminGetUserTransactionListSocket(data);
    },
  },
  computed: {
    ...mapGetters(["modalsData", "adminUserTransactionList"]),
  },
  created() {
