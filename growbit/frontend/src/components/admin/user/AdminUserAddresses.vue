<template>
  <div class="admin-user-addresses">
    ...
    <div class="addresses-content">
      <div class="content-list">
        <AdminUserAddressesElement
          v-for="address in modalsData.user.ips.slice(
            (this.adminPage - 1) * 12,
            this.adminPage * 12
          )"
    ...
    <div class="addresses-pagination">
      <button
        v-on:click="adminSetPage(adminPage - 1)"
        class="button-prev"
        v-bind:disabled="adminPage <= 1"
      >
      ...
      <div class="pagination-info">
        PAGE <span class="text-green-gradient">{{ adminPage }}</span> /
        {{
          Math.ceil(modalsData.user.ips.length / 14) <= 0
            ? "1"
            : Math.ceil(modalsData.user.ips.length / 14)
        }}
      </div>
      <button
        v-on:click="adminSetPage(adminPage + 1)"
        class="button-next"
        v-bind:disabled="
          adminPage >= Math.ceil(modalsData.user.ips.length / 14)
        "
      >
        <div class="button-inner">
          <svg
...
export default {
  ...
  created() {
    console.log(
      this.modalsData.user.ips.slice(
        (this.adminPage - 1) * 12,
        this.adminPage * 12
      )
    );
