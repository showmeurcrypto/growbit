<template>
  <div
    class="battles-box-element"
    v-bind:class="{ 'element-selected': battlesSelectedCount >= 1 }"
  >
    <div class="element-name">
      <div class="name-inner">{{ box.caseName }}</div>
    </div>
    ...
    <div v-if="battlesSelectedCount === 0" class="element-select">
      <div class="select-price">
        <div class="price-value">
          <span>{{ getDisplayCurrencyAmountFormatted(box.casePrice) }}</span>
          <Currency></Currency>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    battlesSelectedCount() {
      return this.battlesSelected.filter(
        (element) => element._id === this.box._id
      ).length;
    },
  },
};
...
.battles-box-element .name-inner {
  ...
  background: linear-gradient(
      0deg,
      rgba(29, 101, 159, 0.08) 0%,
      rgba(29, 101, 159, 0.08) 100%
    ),
    linear-gradient(
      223deg,
      rgba(5, 29, 48, 0.35) 0%,
